...
Las señales computadas o readonly signals, como lo dice su nombre no es mas que un signal de solo lectura. Pero no quiere decir que no pueda cambiar. Si sus dependencias que sean señales cambian, ella cambia.
...
Objeto que permite transformar de manera visual la informacion que mostraremo en pantalla sin mutar la data real. Mas adelante los veremos bien.

    <dd> {{ name() | uppercase }} </dd>

**43. Resumen de lo aprendido**

app:

Contiene todo lo que vamos a estar usando en la aplicacion

pages:

No es obligatorio, pero al profe le gusta poner los componentes que van a ocupar toda la pantalla. Y casi siempre estan referenciadas en app.routes.ts.

app.routes:

Generador de rutas para navegar.

app.component.html:

Contiene el <router-outlet> que es el encargado de mostrar el componente que corresponde segun la ruta en la que nos encontramos

index.html

Contiene el <app-root> que basicamente todo lo que hagamos en la aplicacion esta montado sobre esa etiqueta.

Signals

Es una manera actualizada de cambiar el estado de la aplicacion. No significa que solo podemos colocar primitivos, sino tambien objetos, arreglos y cualquier estructura de datos.

**44. Codigo fuente de la seccion**
